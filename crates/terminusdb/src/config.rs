...
impl TerminusConfig {
    pub fn from_env() -> Result<Self, TerminusError> {
        ...
        let port: u16 = std::env::var("TERMINUSDB_PORT")
            ...
            .parse()
            .map_err(|_| TerminusError::Config("TERMINUSDB_PORT must be a valid u16".into()))?;
        ...
        let password = std::env::var("TERMINUSDB_ADMIN_PASS")
            .map_err(|_| TerminusError::Config("TERMINUSDB_ADMIN_PASS must be set".into()))?;

        let base_url = Url::parse(&format!("http://{host}:{port}"))
            .map_err(|e| TerminusError::Config(format!("invalid base URL: {e}")))?;
